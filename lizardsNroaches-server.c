...
void process_display_app_message(message_to_server *recv_message)
{
    // TODO
}
...
void process_roach_connect(variable_that_roach_need *roach_payload)
{
    ...
    wmove(roach_payload->game_window, roach_payload->roaches[id].x, roach_payload->roaches[id].y);
    waddch(roach_payload->game_window, (roach_payload->roaches[id].ch + 48) | A_BOLD);
    wrefresh(roach_payload->game_window);
...
void process_roach_movement(variable_that_roach_need *roach_payload)
{
    ...
    wmove(roach_payload->game_window, roach_payload->roaches[id].x, roach_payload->roaches[id].y);
    waddch(roach_payload->game_window, ' '); // MIGHT CAUSE A PROBLEM
...
int main()
{
    ...
    while (1)
    {
        ...
        case DISPLAY_APP:
            ...
            process_display_app_message(&recv_message);
            break;
        default:
            break;
        }
